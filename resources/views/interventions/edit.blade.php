                        ...
                        <select name="registrationNum" id="registrationNum" class="form-control @error('registrationNum') form-error-input @enderror">
                            @if($intervention->technician != null)
                                @forelse($technicians as $technician)
                                    <option value="{{ $technician->id }}" {{ $intervention->technician->id === $technician->id ? 'selected' : '' }}>{{ $technician->id . ' - ' . $technician->employee->firstName . ' ' . $technician->employee->lastName }}</option>
                                @empty
                                    <option value="-1">No technicians available</option>
                                @endforelse
                            @else
                                <option value="-1">No technician assigned</option>
                            @endif
                        </select>
